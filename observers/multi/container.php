...
const CONTAINER_EXTENSIONS = array('pdo_mysql', 'sockets', 'pcntl', 'posix');

// This image never copies http_server in, so it has no served tree at all.
const CONTAINER_CODE_PATHS = array(
    '/pr2/config.php',
    '/pr2/common',
    '/pr2/functions',
    '/pr2/multiplayer_server',
    '/pr2/vend',
    '/pr2/observers/multi',
);
...
// No document root and no web server here.
const CONTAINER_DOCUMENT_ROOT = '';
const CONTAINER_APACHE_MODULES = array();

const CONTAINER_PROCESS = 'pr2.php';

// The player port the client dials, and the control port, which is never
// published and carries the signed commands.
const CONTAINER_PORTS = array('player' => 9160, 'control' => 9161);
