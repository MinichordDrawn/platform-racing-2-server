...
try {
    ...
    // sanity: switched off, or the poller cannot reach it?
    if ((int) $server->active === 0 || strtolower((string) $server->status) === 'down') {
    ...
    $result = talk_to_server("multi"/*$server_address*/, $server_port, $server->salt, $str, true, false);
